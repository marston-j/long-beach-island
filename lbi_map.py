...
/* --app-vh is set from window.innerHeight, which is the only figure that
   tracks iOS Safari's collapsing toolbar. dvh covers browsers where the
   script has not run yet; plain vh is the last resort. */
.shell{display:flex;overflow:hidden;
  height:100vh;height:100dvh;height:calc(var(--app-vh,1dvh)*100)}
...
@media(max-width:760px){
  ...
  .sidebar{position:absolute;left:0;right:0;bottom:0;z-index:1200;width:auto;
    ...
    max-height:calc(var(--app-vh,1dvh)*86);height:calc(var(--app-vh,1dvh)*86);
    border-right:none;border-top:1px solid #d9d3c9;
...
/* Track the genuinely visible height. iOS Safari grows and shrinks its
   toolbar as you scroll and on rotation, and vh does not follow it. */
function setAppVH(){
  var h=(window.visualViewport&&window.visualViewport.height)||window.innerHeight;
  document.documentElement.style.setProperty('--app-vh',(h*0.01)+'px');
  if(typeof _map!=='undefined'&&_map)_map.invalidateSize();
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  /* Safari reports a stale height while the toolbar animates. */
